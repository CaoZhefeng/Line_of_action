import numpy as np
from game import TwoPlayersGameState, Action
...
class TicTacToeGameState(TwoPlayersGameState):
	...
	def __init__(self, state, next_to_move=1):
		if len(state.shape) != 2 or state.shape[0] != state.shape[1]:  # 棋盘为二维的
			raise ValueError("Please play on 2D square board")
		self.board = state
		self.board_size = state.shape[0]  # 棋盘边长作为度量
		self.next_to_move = next_to_move

	@property
	def game_result(self):
		# check if game is over
		rowsum = np.sum(self.board, 0)  # 横向成行
		colsum = np.sum(self.board, 1)  # 纵向成列
		# 斜线条件
	...
	def move(self, move):
		...
		next_to_move = TicTacToeGameState.o if self.next_to_move == TicTacToeGameState.x else TicTacToeGameState.x  # 交替下子
